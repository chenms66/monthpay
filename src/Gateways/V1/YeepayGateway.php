...
class YeepayGateway extends AbstractGateway
{
    ...
    /** API 路径 */
    const API_FAST_BIND_CARD_REQUEST = '/rest/v1.0/frontcashier/fastbindcard/request';
    const API_TRADE_ORDER = '/rest/v1.0/trade/order';
    const API_BIND_PAY_REQUEST = '/rest/v1.0/frontcashier/bindpay/request';
    const API_BIND_PAY_CONFIRM = '/rest/v1.0/frontcashier/bindpay/confirm';
    const API_BIND_CARD_REQUEST = '/rest/v2.1/frontcashier/bindcard/request';
    const API_BIND_CARD_CONFIRM = '/rest/v2.0/frontcashier/bindcard/confirm';
    ...
    public function tradeOrder(array $params)
    {
        ...
        $data['notifyUrl'] = $this->config['callback'] ?? '';
        return $this->request($data, self::API_TRADE_ORDER, __FUNCTION__);
    }
     ...
     * 第三步：绑卡支付-支付下单
     */
    public function bindPayRequest(array $params)
    {
        Validator::validateRequiredFields($params, [
            'token',
            'bind_id',
        ...
        $data['version'] = '1.0';
        $data['extParamMap'] = $params['ext_param_map'] ?? '';
        $data['payMerchantNo'] = $params['pay_merchant_no'] ?? '';

        return $this->request($data, self::API_BIND_PAY_REQUEST, __FUNCTION__);
    }

    /**
     * 第四步：绑卡支付-确认支付
     */
    public function bindPayConfirm(array $params)
    {
        Validator::validateRequiredFields($params, [
            'token',
        ]);
        ...
        $data['token'] = $params['token'];
        $data['version'] = '1.0';
        $data['verifyCode'] = $params['verify_code'] ?? '';
        $data['owner'] = $params['t_name'] ?? '';
        $data['idno'] = $params['t_paper_num'] ?? '';
        $data['phoneNo'] = $params['t_tel'] ?? '';
        $data['avlidDate'] = $params['avlid_date'] ?? '';
        $data['cvv2'] = $params['cvv2'] ?? '';
        $data['idCardType'] = $params['id_card_type'] ?? '';
        $data['bankPWD'] = $params['bank_pwd'] ?? '';

        return $this->request($data, self::API_BIND_PAY_CONFIRM, __FUNCTION__);
    }
    ...
    public function bindCardRequest(array $params)
    {
        ...
        $data['authType'] = $params['card_type'] == self::DEBIT_CARD ? 'COMMON_FOUR' : 'CREDIT_SIX';
    ...
    {
        $request = $this->tradeOrder($params);
        if($request['code'] == self::TOKEN_SUCCESS){
            $params['token'] = $request['token'];
            $request = $this->bindPayRequest($params);
            if($request['code'] == self::PAY_CONFIRM){
                $request = $this->bindPayConfirm($params);
            }
        }
        return $request;
    }
